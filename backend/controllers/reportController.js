...
// GET /api/v1/reports/summary - Key sales metrics & top items
const getSalesSummary = async (req, res) => {
  try {
    // 1. Total revenue & order counts (filters by completed/paid status)
    const summaryQuery = `
      SELECT 
    ...
    // 2. Top 5 selling menu items joining orders and order_items
    const topItemsQuery = `
      SELECT 
        m.name, 
